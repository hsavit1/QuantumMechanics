...
ArrayXXd HermitianEigenSolver::eigenvalues(EigenSystem::range range) const
{
	...
	const char range_token = (range.value == EigenSystem::range::full_range) ? 'A' : ((range.value == EigenSystem::range::value_range) ? 'V' : 'I' );
	...
	{
		MatrixXcd M = (M_array) ? M_array[p_index] : ( (M_function)? M_function(p_index) : MatrixXcd() );
		...
		lapack_int info = LAPACKE_zheevr( major_dim_order, job_token, range_token, upper_lower_token, order, reinterpret_cast<MKL_Complex16*>(M.data()), major_dim_length, range.lowest_value, range.highest_value, range.begin_index, range.end_index, 0., &value_count, values.data()+p_index*order, nullptr, 0, suppliements);
...
ArrayXd HermitianEigenSolver::eigenvalues(const MatrixXcd &M, EigenSystem::range range)
{
	...
	lapack_int info = LAPACKE_zheevr( major_dim_order, job_token, range_token, upper_lower_token, order, reinterpret_cast<MKL_Complex16*>(M.data()), major_dim_length, range.lowest_value, range.highest_value, range.begin_index, range.end_index, 0., &value_count, values.data(), nullptr, 0, suppliements);
