...
struct range {
	...
	range(range_type type, long begin, long end) :
		value(type),
		begin_index((type == index_range || type == mid_index_range) ? begin : 0),
		end_index((type == index_range || type == mid_index_range) ? end : 0),
		lowest_value((type == value_range) ? begin : 0),
		highest_value((type == value_range) ? end : 0)
	{ };


	range(range_type type, double lowest, double highest) :
		value(type),
		begin_index((type == index_range || type == mid_index_range) ? lowest : 0),
		end_index((type == index_range || type == mid_index_range) ? highest : 0),
		lowest_value((type == value_range) ? lowest : 0),
		highest_value((type == value_range) ? highest : 0)
	{ };
...
public:
	...
	HermitianEigenSolver(size_t n, const std::function<MatrixXcd(int)> &M, const size_t &order);
	HermitianEigenSolver(const std::vector<MatrixXcd> &M);
	virtual ~HermitianEigenSolver();

	ArrayXXd eigenvalues(EigenSystem::range range = EigenSystem::range::full()) const;

	static ArrayXd eigenvalues(const MatrixXcd &M, EigenSystem::range range = EigenSystem::range::full());
